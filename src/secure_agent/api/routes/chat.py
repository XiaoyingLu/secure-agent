...
from secure_agent.auth.rbac import AGENT_USER_ROLE_NAME, require_role
...
def _extract_user_token(request: Request) -> str:
    ...
    try:
        token = extract_delegated_token(user)
        logger.debug("Delegated token extracted from request state.")
        return token
...
async def post_chat(
    ...
    service = ChatService()
    try:
        result = await service.process_chat_async(
            agent=agent,
