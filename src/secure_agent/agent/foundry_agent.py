...
from secure_agent.auth.obo_client import OBOClient
...
class FoundryAgent:
    ...
    async def chat(
        ...
        # OBO exchange is deferred ΓÇö only performed when tools are actually called.
        ...
        while True:
            ...
            if not function_outputs:
                # No more tool calls ΓÇö model has produced its final answer
                break
...
def _parse_enabled_tools_from_env() -> frozenset[str]:
    """Return the normalized tool allowlist from the environment."""
    enabled_env = os.getenv("ENABLED_TOOLS", "").strip()
    return frozenset(t.strip() for t in enabled_env.split(",") if t.strip())


def discover_base_tools(
    ...
    comma-separated allowlist are included. The optional ``enabled_tool_names``
    parameter preserves compatibility with the legacy discovery API used by the
    tests.
    """
    import secure_agent.tools as tools_package
    ...
    if enabled_tool_names is not None:
        allowlist = frozenset(enabled_tool_names)
    else:
        env_allowlist = _parse_enabled_tools_from_env()
        allowlist = env_allowlist or None
...
def _describe_extra_tools(tools: Iterable[BaseTool]) -> str:
    ...
    for tool in extra:
        lines.append(f"- **{tool.name}** ΓÇö {tool.description}")
    lines.append(
